...
# Davidi
# unit mmol/gCDW
rxn_con2abundance_davidi= dict()
with open('../data/davidi/abundance_by_reaction.mmol_gCDW.csv', 'r') as file:
    reader= csv.reader(file, delimiter= ',')
    abundance_header_davidi= next(reader, None)[1:]
    for row in reader:
        rxn= row[0].replace("_reverse", "_b")
        for con_index, condition in enumerate(abundance_header_davidi):
            if row[con_index+1] != '':
                abundance= float(row[con_index+1])
                rxn_con2abundance_davidi[(rxn, condition)]= abundance
            
print("Davidi Abundance:  {}, \tMean:  {}".format(len(rxn_con2abundance_davidi),
...
con2fluxes_davidi= dict()
with open('../data/davidi/davidi_fluxes_pFBA.csv', 'r') as file:
    reader= csv.reader(file, delimiter= ',')
    for row in reader:
        con= row[0]
        flux_values= [float(flux) for flux in row[1:-1]]
        con2fluxes_davidi[con]= flux_values
        
cons= []
models_dir= '../data/GEMs/modified_models_davidi_irrev/'
for file_name in os.listdir(models_dir):
    target_con= '.'.join(file_name.replace("iJO1366_irrev_", "").split(".")[0:-1])
    cons.append(target_con)
...
for target_con in cons:
    ...
    model= loadmat(f'../data/GEMs/modified_models_davidi_irrev/iJO1366_irrev_{target_con}.mat')['modelIrrev']

    mets= [element[0][0] for element in model['mets'][0][0]]
    nMets= len(mets)
    rxns= [element[0][0] for element in model['rxns'][0][0]]
    nRxns= len(rxns)
    S = model['S'][0][0].todense()
    ...
    for rxn in target_rxns:
        if (rxn, target_con) in rxn_con2abundance_davidi:
    ...
    if (m.status== GRB.OPTIMAL or m.status == GRB.TIME_LIMIT) and m.solCount > 0:
        ...
        correlation= np.corrcoef(np.log(v_measured+1e-4), np.log(v.x+1e-4))[0, 1]
